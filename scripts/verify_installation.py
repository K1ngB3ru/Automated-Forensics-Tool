...
def check_directory_structure():
    ...
    for dir_path, friendly_name in required_dirs:
        if dir_path.exists():
            ...
            print_warning(f"{friendly_name}: {dir_path} (Will be created on first run)")
...
def check_memory_tools():
    ...
    winpmem_paths = [
        TOOLS_DIR / "winpmem" / "winpmem_mini_x64_rc2.exe",
        TOOLS_DIR / "winpmem" / "winpmem.exe",
    ]
...
def check_process_tools():
    ...
    sysinternals_tools = [
        (TOOLS_DIR / "sysinternals" / "procmon.exe", "Process Monitor"),
        (TOOLS_DIR / "sysinternals" / "procexp.exe", "Process Explorer"),
        (TOOLS_DIR / "sysinternals" / "autoruns.exe", "Autoruns"),
        (TOOLS_DIR / "sysinternals" / "tcpview.exe", "TCPView"),
        (TOOLS_DIR / "sysinternals" / "strings.exe", "Strings"),
    ]
...
def check_debugging_tools():
    ...
    ghidra_dirs = list((TOOLS_DIR / "ghidra").glob("ghidra_*"))
    if ghidra_dirs:
    ...
    x64dbg_paths = [
        TOOLS_DIR / "x64dbg" / "release" / "x64" / "x64dbg.exe",
        TOOLS_DIR / "x64dbg" / "x64dbg.exe",
    ]
...
def generate_report(all_results):
    ...
    report_path = LOGS_DIR / f"verification_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
